# Security Model — ProAppStore Storefront

This document describes the security posture of the ProAppStore storefront
(everything served from `proappstore.online`). It's intentionally short — the goal is
...
`registry.json` (apps (with `proFeatures` array)). There is no server-side render, no
...
| Image / fetch exfiltration via CSP gap  | Any HTTPS host                 | `img-src` and `connect-src` allowlisted to `*.proappstore.online`. No blanket `https:`.                                              |
...
No client-side OAuth today — sign-in flows through `dashboard.proappstore.online` (separate property).
...
3. **`'unsafe-hashes'` not used for `style="..."` attributes** because there
   ...
   catches it.
4. **Detail pages fetch `api.proappstore.online/v1/storefront/apps/<id>`** to progressively enhance. The fetch is allowlisted in CSP `connect-src`. Schema validation of the response happens client-side in `detail-page.js`.
