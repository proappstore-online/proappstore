...
const features = '<section style="margin:2rem 0 3rem"><h2 style="font-size:1.3rem;font-weight:800;margin-bottom:1rem">Platform features</h2><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:1rem">' +
  ...
  '<div style="padding:1rem;border:1px solid var(--border);border-radius:0.75rem"><strong>Maps + Geocoding</strong><p style="font-size:0.8rem;color:var(--muted);margin-top:0.25rem">Geocode addresses, embed maps, no Google API keys. OpenStreetMap powered.</p></div>' +
  '<div style="padding:1rem;border:1px solid var(--border);border-radius:0.75rem"><strong>ProShell + Hooks</strong><p style="font-size:0.8rem;color:var(--muted);margin-top:0.25rem">Platform auth gate + subscription wall. ProShell or hooks -- you control the UI.</p></div>' +
